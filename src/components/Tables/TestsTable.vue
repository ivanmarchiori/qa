<template>
  <div>
    <md-table v-model="testsData" :table-header-color="tableHeaderColor">
      <md-table-row slot="md-table-row" slot-scope="{ item }">
        <md-table-cell md-label="Tipo de Teste">{{
          item.tipoTeste
        }}</md-table-cell>
        ...
        <md-table-cell md-label="Quantidade Registros">{{
          item.qtdRegistros
        }}</md-table-cell>
        ...
        <md-table-cell md-label="Executar"
          ><button
            type="button"
            class="md-button md-raised md-danger md-theme-default"
          >
            <div class="md-ripple">
...
export default {
  ...
  mounted() {
    fetch("http://localhost/qa/api/?metodo=tests")
      .then((res) => res.json())
      .then((data) => (this.testsData = data.data))
